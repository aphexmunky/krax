...
class UserService extends Actor with ActorLogging {
	...
	val idExtractor: ShardRegion.IdExtractor = {
		case msg @ Register(username)		⇒ (username, msg)
		case get @ GetEmail(username)		⇒ (username, get)
		case add @ AddEmail(username, _)	⇒ (username, add)
	}

	val shardResolver: ShardRegion.ShardResolver = {
		case Register(username)				⇒ (username.hashCode % 12).toString
		case GetEmail(username)				⇒ (username.hashCode % 12).toString
		case AddEmail(username, _)			⇒ (username.hashCode % 12).toString
	}
